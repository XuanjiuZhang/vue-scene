<template>
  <div>
    <div class="option-group" :style="eleData.properties.optionStyle">
      ...
      <ul>
        <li v-for="(option, index) in eleData.properties.options" @click="toggle(index)" :style="optionItemStyle">
          <div class="check-radio" :style="{'border-color': eleData.properties.optionStyle.backgroundColor}">
            <span class="check-radio-dot" v-show="option.selected" :style="{'background-color': eleData.properties.optionStyle.backgroundColor}"></span>
          </div>
          {{option.text}}
        </li>
      </ul>
